steps_for(:login) do
  ...
  When "the user logs in with the correct username and password" do
    u = User.new
    u.login, u.password, u.password_confirmation, u.email = @username, @password, @password, 'foo@bar.com'
    u.save
    post "/sessions",  :login => @username, :password => @password
  end
  ...
  Then "the menu list page should be shown" do
