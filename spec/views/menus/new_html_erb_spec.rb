...
describe 'menus/new' do
  ...
  before(:each) do
    ...
    render('/menus/new', :layout => 'application')
  end
  ...
  it 'should have a form with action "/menus/new" and method="post"' do
    response.should have_tag('form', :attributes => {:action => '/menus', :method => 'post'})
  end
  
  describe '/menus form' do

    it 'should have a input tag of type text and id menu_name' do
      response.should have_tag('form', :attributes => {:action => '/menus', :method => 'post'}) do
